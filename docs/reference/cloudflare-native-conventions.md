...
- Billable-metric update/delete first acquires a transaction-local D1 mutation guard, then writes
  the optimistic aggregate transition and exactly one outbox row in the same batch. Deletion
  retires attached charges and enqueues one durable cleanup owner; the five-minute Workflow
  tombstones events and drains R2 intents in bounded batches. Retired metrics disappear from event
  and wallet-target reads immediately, while deterministic metric generations preserve historical
  IDs and permit same-code recreation.
...
| Payment request           | payment-request API and later dunning owner                                                   | request, version-pinned overdue invoice links, and outbox                                                                 | tenant/customer/currency/readiness checks; invoice version/status trigger; one D1 batch                                              |
| Dunning campaign          | campaign API, customer API, and hourly `:45` reconciliation owner                             | campaign/default/threshold changes or one request/invoice-link/customer-attempt/outbox transition                         | tenant relationship triggers; customer version guard; deterministic campaign/threshold/request IDs; elapsed-attempt predicate        |
...
| Schedule run              | one-minute dispatcher and reconciliation Workflow                                             | slot audit and each executor's own aggregate batch                                                                        | deterministic slot/instance ID; unique run ID; due-but-unported work recorded as partial                                             |
