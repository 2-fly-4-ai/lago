...
INSERT INTO plans (id,organization_id,code,name,interval,amount_minor,currency,version,active,created_at,updated_at,pay_in_advance,metadata_json,pending_deletion)
SELECT 'catalog-2200da3645cd79bcfd7a40bc948b3e3d','org-serp-billing','serp-downloaders-bundle-monthly','SERP Downloaders Bundle Monthly','monthly',7900,'USD',1,1,strftime('%Y-%m-%dT%H:%M:%fZ','now'),strftime('%Y-%m-%dT%H:%M:%fZ','now'),1,'{"source":"store-new","source_commit":"d01233ffdc9718ee7e89fe6250c6e163d4e41172","catalog_repair":"2026-09-05"}',0
WHERE NOT EXISTS (SELECT 1 FROM plans WHERE organization_id='org-serp-billing' AND code='serp-downloaders-bundle-monthly' AND parent_id IS NULL);
INSERT INTO plans (id,organization_id,code,name,interval,amount_minor,currency,version,active,created_at,updated_at,pay_in_advance,metadata_json,pending_deletion)
SELECT 'catalog-be6912a659cd10a0163d8150c793016e','org-serp-billing','serp-downloaders-bundle-yearly','SERP Downloaders Bundle Yearly','yearly',87900,'USD',1,1,strftime('%Y-%m-%dT%H:%M:%fZ','now'),strftime('%Y-%m-%dT%H:%M:%fZ','now'),1,'{"source":"store-new","source_commit":"d01233ffdc9718ee7e89fe6250c6e163d4e41172","catalog_repair":"2026-09-05"}',0
WHERE NOT EXISTS (SELECT 1 FROM plans WHERE organization_id='org-serp-billing' AND code='serp-downloaders-bundle-yearly' AND parent_id IS NULL);
